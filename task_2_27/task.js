...
var commander = (() => {  
  ...
  let addNewSpaceShip = (...args) => {
    ...
    if(typeof token === "number") {
      let obj = args.shift();
      obj.id = token;
      mediator({args: obj, command: "addPlayer"});
    } else {
      return false;
    }
  };
  ...
  let sendCommand = ({args, command}) => {
    if(command === "remove") {
      ...
      mediator({args: args, command: "remove"});
    } else {
      mediator({args: args, command: command});
    }
  };
...
var mediator = (() => {
  ...
  let succeed = ({spaceship, command}) => {
    if (parseInt(command.slice(0, 4), 2) === spaceship.id &&
      command.slice(-4) === "0010") {
      let token = spaceship._token;
      players = players.filter(obj => obj._token !== token);
    }
  };

  // BUS 用于模拟真空, 提供参数传递时的丢包、延迟效果
  let BUS = ({binaryId, binaryCommand}) => {
    let broadCast = ({command, spaceship}) => {
      setTimeout(() => {
        console.log("singal received!");
        if(Math.random() > 0.1) {
          console.log("singal executed!");
          spaceship.receiver(command);
          succeed({spaceship: spaceship, command: command});
        } else {
          // 失败了的话, 就重新传输一次这个参数
          console.log("BUS failed.");
          broadCast({command: command, spaceship: spaceship});
          console.log("command resent.");
        }
      }, config.delay);
    };

    players.forEach(e => broadCast({command: binaryId + binaryCommand, spaceship: e}));
  };
